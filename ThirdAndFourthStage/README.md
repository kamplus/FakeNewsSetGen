...
## This code was adapted from FakeNewsNet (Kai Shu, 2018) {FakeNewsNet: A Data Repository with News Content, Social Context and Dynamic Information for Studying Fake News on Social Media}
...
The minimalistic version of latest dataset provided in this repo (located in `dataset` folder) include following files:
 ...
 - `News_fake.csv` -  Samples related to fake news collected from Fact-checking Agencies and Renowned Virtual Media
 - `News_notFake.csv` -  Samples related to news collected from Fact-checking Agencies and Renowned Virtual Media
...
Inside the dataset folder are the two files resulting from the execution of the first built FakeNewsSet (composed of 300 fake and 300 not-fake news). If the first and second stages are run again, the files .\FirstAndSecondStage\Dataset\News_fake.csv and .\FirstAndSecondStage\Dataset\News_notFake.csv will be updated with new news. Therefore, the corresponding files stored in the .\ThirdAndFourthStage\Dataset folder must be replaced.
...
The `config.json` can be used to configure and collect only certain parts of the dataset. Following attributes can be configured    
 ...
 - **data_collection_choice** - It is an array of choices of various parts of the dataset. Configure accordingly to download only certain parts of the dataset.       
   Available values are  
     {"news_source": "News", "label": "fake"},{"news_source": "News", "label": "notFake"}
